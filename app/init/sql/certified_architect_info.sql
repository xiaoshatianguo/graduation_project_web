CREATE TABLE IF NOT EXISTS `certified_architect_info` (
  ...
  `name` VARCHAR(255) DEFAULT '' comment '认证师真实名字',
  `sex` INT(2) DEFAULT 0 comment '认证师性别 - 0 男 - 1 女',
  `age` INT(2) DEFAULT 0 comment '认证师年龄',
  `address` VARCHAR(255) DEFAULT 0 comment '认证师地址',
  -- TODO: 这里需要更换默认值为默认头像
  ...
  `state` INT(11) DEFAULT 0 comment '认证师账号状态 - 0 未激活 - 1 正常 - 2 禁用',
  PRIMARY KEY (`uuid`)
